...
if ($_FILES["itemImg"]["error"] === 0) {
    ...
    if (move_uploaded_file($_FILES["itemImg"]["tmp_name"], "../ma/images/items/{$itemImg}")) {
...
if ($stmt->rowCount() > 0) {
    header("Refresh: 1; url=./admin.php?itemId={$_POST['itemId']}");
    $objResponse['success'] = true;
    $objResponse['info'] = "更新成功";
    echo json_encode($objResponse, JSON_UNESCAPED_UNICODE);
    exit();
} else {
    header("Refresh: 1; url=./edit.php?itemId={$_POST['itemId']}");
